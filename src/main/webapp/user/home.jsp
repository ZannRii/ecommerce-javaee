...
<body class="bg-light">
	...
	<div class="position-sticky top-0 z-3 shadow-sm">
		...
		<header class="navbar navbar-expand-lg navbar-dark bg-dark px-3 px-lg-4 py-3">
		...
		<nav class="bg-white border-bottom px-3 px-lg-4 py-2 d-flex gap-2 overflow-auto">

			<button class="btn btn-outline-success btn-sm rounded-pill px-3" onclick="location.href='home'">All</button>
	...
	<section class="bg-success bg-gradient text-white py-5">
		<div class="container py-4">
			<div class="row align-items-center g-4">
				<div class="col-lg-7">
					<p class="text-uppercase fw-bold text-warning mb-2">Seasonal deals</p>
					<h1 class="display-4 fw-bold">Shop smarter with fresh picks</h1>
					<p class="lead mb-0">Curated products, clear prices, and fast checkout for everyday shopping.</p>
				</div>
				<div class="col-lg-5 text-lg-end">
					<img src="${pageContext.request.contextPath}/images/laptop.png" class="img-fluid rounded-4 shadow" alt="Featured product">
				</div>
			</div>
		</div>
	</section>
	...
	<section class="container py-5">
		...
		<div class="card h-100 border-0 shadow-sm">
			...
			<img src="<%=p.getImageUrl()%>" class="card-img-top object-fit-cover" height="210" alt="<%=p.getName()%>">
			...
			<form action="cart" method="post" class="mt-auto">
				...
				<input class="form-control mb-2" type="number" name="quantity" value="1" min="1"
					max="<%=p.getStockQuantity()%>">
				<button class="btn btn-success w-100 fw-semibold" type="submit"><i class="bi bi-cart-plus me-1"></i>Add To Cart</button>

			</form>
			<a href="product-detail?id=<%=p.getProductId()%>"
				class="btn btn-outline-dark w-100 mt-2"> View Details <i class="bi bi-arrow-right"></i></a>
			</div>
		</div>
		</div>
